...
d_ET = df_diff[band != "GPP", .(band = "ET", value = sum(value, na.rm = TRUE)), .(I)]
df_diff2 <- rbind(df_diff, d_ET)
...

{
    ...
    ps = foreach(bandName = c("GPP", "ET", "Ec", "Es", "Ei"), i = icount()) %do% {
        if (i == 1) {
        ...
        p <- levelplot2(value ~ s1+s2 | band,
                    ...
                    aspect = 0.5,
                    panel.title = sprintf("(%s) %s", letters[i], bandName),
    ...
    write_fig(g, "Figure2_GPP_ET_dynamic-static (2004-2017)_rep_poly_20190121.png", 9.3, 6)
}
