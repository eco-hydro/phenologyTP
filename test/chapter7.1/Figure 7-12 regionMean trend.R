...
{
    lst_id <- overlap_id(grid_, TP_poly_veg)
    d_id <- map(lst_id[-c(7, 10)], ~ data.table(I = .x)) %>% melt_list("region")
}

## get regional mean
get_regional_mean <- function(l, lst_id) {
    ans <- foreach(ind = lst_id, i = icount()) %do% {
        map(l, ~.[ind, ]) %>% colMeans2()
    }
    ans
}

{
    ...
    temp <- foreach(l = lst_pheno, i = icount()) %do% {
        ...
        res = foreach(LAI = l_LAI) %do% {
            ...
            foreach(k = seq_along(Y) %>% set_names(names(Y))) %do% {
                l <- c(Y[k], X)
                # ans <- foreach(i = indexes, icount()) %do%
                #     {
                #         d <- map(l, ~ .x[i, ]) %>% as.data.table() %>% na.omit()
    ...
    #     list(coef = l_pcor, pvalue = l_pvalue)
    # }
